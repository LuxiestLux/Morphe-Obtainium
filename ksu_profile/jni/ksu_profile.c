...
int main(int argc, char* argv[]) {
    ...
    if (!ksuctl(KSU_IOCTL_GET_MANAGER_APPID, &appid_cmd)) {
        fprintf(stderr, "ksu_profile: GET_MANAGER_APPID failed: %s\n", strerror(errno));
        return 1;
    }

    if (setuid(appid_cmd.appid) != 0) {
        fprintf(stderr, "ksu_profile: setuid(%u) failed: %s\n", appid_cmd.appid, strerror(errno));
        return 1;
    }
    ...
    if (!ksuctl(KSU_IOCTL_GET_APP_PROFILE, &profile)) {
        ...
        strncpy(profile.key, argv[2], sizeof(profile.key) - 1);
    }
    ...
    if (!ksuctl(KSU_IOCTL_SET_APP_PROFILE, &profile)) {
        fprintf(stderr, "ksu_profile: SET_APP_PROFILE failed for %s: %s\n", argv[2], strerror(errno));
        return 1;
    }
